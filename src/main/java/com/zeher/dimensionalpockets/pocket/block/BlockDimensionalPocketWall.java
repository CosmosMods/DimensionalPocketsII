...
public class BlockDimensionalPocketWall extends ModBlock {
	
	public BlockDimensionalPocketWall(String name, Material arg0, String tool, int harvest, CreativeTabs tab) {
		...
		if (playerIn.dimension == DimensionalPockets.DIMENSION_ID) {
			...
			if (!playerIn.isSneaking()) {
				if (ModUtil.isHoldingHammer(playerIn)) {
					...
					worldIn.setBlockState(pos, BlockHandler.BLOCK_DIMENSIONAL_GLASS.getDefaultState());
					return true;
				}
			} else {
				if (ModUtil.isHoldingHammer(playerIn)) {
					...
					worldIn.setBlockState(pos, BlockHandler.BLOCK_DIMENSIONAL_POCKET_WALL_CONNECTOR.getDefaultState());
					return true;
				}
