...
public class GuiConnector extends GuiContainerElements {

	private TileConnector tile;
	...
	private int[] ENERGY_BAR = new int[] { 11, 187 };
	private int[] ENERGY_BAR_ = new int[] { 72, 160, 58, 18 };
	private int[] FLUID_BAR = new int[] { 37, 52, 188, 243, 18, 58 };
	...
	private int[] TABI = new int[] { 61, 227, 18 };
	
	private GuiTextField TEXT_FIELD;
	private int[] TEXT_FIELDI = new int[] { 92, 17, 100, 16 };
	private int[] ELEMENT_LISTI = new int[] { 92, 46, 140, 0 };
	...
	/** Type Button */
	private GuiDimensionalButton TYB;
	private int[] TYBI = new int[] { 11, 16, 20 };
	
	public GuiConnector(InventoryPlayer inventoryPlayer, TileConnector tileEntity) {
		super(new ContainerConnector(inventoryPlayer, tileEntity));
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		DRAW.bindTexture(this, DimReference.GUI.RESOURCE.GUI_CONNECTOR);
		DRAW.drawStaticElement(this, screen_coords, 0, 0, 0, 0, 256, 256);
		SCROLL.drawScrollElement(this, screen_coords, 239, 46, this.current_scroll, true, 2, false);
		
		//System.out.println(this.tile.getEnergyScaled(58) + " :: " + this.tile.getEnergyStored(EnumFacing.DOWN) + " :: " + this.tile.getMaxEnergyStored(EnumFacing.DOWN));
		
		DRAW.drawPowerBar(this, screen_coords, ENERGY_BAR[0], ENERGY_BAR[1], this.tile.getEnergyScaled(58), ENERGY_BAR_, this.tile.hasEnergy());
		DRAW.drawFluidTank(this, screen_coords, FLUID_BAR[0], FLUID_BAR[2] + 22, this.tile.getTank(), this.tile.getFluidLevelScaled(60));
		
		DRAW.bindTexture(this, DimReference.GUI.RESOURCE.GUI_CONNECTOR_SLOTS);
		DRAW.drawStaticElement(this, screen_coords, 256, 0, 0, 0, 92, 256);
	}

	@Override
	protected void actionPerformed(GuiButton button) {
		TileEntity tile_pocket = tile.getPocket().getSourceBlockWorld().getTileEntity(tile.getPocket().getLastPos());
		int dim = tile.getPocket().getSourceBlockDim();
		...
		if (button.equals(this.TPB)) {
			...
			if (!(this.tile.getPocket().getPlayerMap().get(this.getSelectedElementId()).equals(this.tile.getPocket().getCreator()))) {
			...
			ModNetworkManager.sendConnectorLock(tile.getPos(), lock);
		} 
		
		else if (button.equals(this.TACB)) {
			if (this.isShiftKeyDown()) {
				ModNetworkManager.sendConnectorEmpty(tile.getPos());
			}
		} 
		
		else if (button.equals(this.TYB)) {
			tile.cycleType();
			ModNetworkManager.sendConnectorType(tile.getPos());
		} 
		
		else if (button.equals(this.SIB)) {
			tile.cycleSide(EnumFacing.DOWN);
			ModNetworkManager.sendConnectorSide(tile.getPos(), EnumFacing.DOWN);
		} 
		
		if (tile_pocket != null && tile_pocket instanceof TilePocket) {
			if (button.equals(this.UB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.UP);
			} else if (button.equals(this.DB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.DOWN);
			} else if (button.equals(this.NB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.NORTH);
			} else if (button.equals(this.SB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.SOUTH);
			} else if (button.equals(this.WB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.WEST);
			} else if (button.equals(this.EB)) {
				ModNetworkManager.sendPocketCycleSideIn(tile_pocket.getPos(), dim, EnumFacing.EAST);
			}
		}
	}
	...
	private void drawButtons(int[] screen_coords) {
		...
		if (tile_pocket instanceof TilePocket) {
			for(EnumFacing c : EnumFacing.VALUES) {
				sides[c.getIndex()] = (((TilePocket) tile_pocket).getSide(c).getIndex() + 6);
			}
		}
		...
		this.SIB = this.addButton(new GuiDimensionalButton(9, screen_coords[0] + SIBI[0], screen_coords[1] + SIBI[1], 20, tile.getSide(EnumFacing.UP).getIndex() + 14, true, true));
		
		this.TYB = this.addButton(new GuiDimensionalButton(10, screen_coords[0] + TYBI[0], screen_coords[1] + TYBI[1], 20, (tile.getType().getIndex() + 10), true, true));
	...
	protected void drawHoveringText(int mouse_x, int mouse_y, int[] screen_coords) {
		if (IS_HOVERING.is(screen_coords, true, mouse_x, mouse_y, ENERGY_BAR[0], ENERGY_BAR[0] + ENERGY_BAR_[3], ENERGY_BAR[1], ENERGY_BAR[1] + ENERGY_BAR_[2])) {
		...
		String[] type = new String[] { 
				TextHelper.PURPLE + TextHelper.BOLD +  "Change interface type" + TextHelper.END, 
				TextHelper.LIGHT_BLUE + "Current Type: " + tile.getType().getTextColour() + 
				"[" + tile.getType().getDisplayName() + "]" + TextHelper.END };
		
		if (IS_HOVERING.is(screen_coords, true, mouse_x, mouse_y, TYBI[0], TYBI[0] + TYBI[2], TYBI[1], TYBI[1] + TYBI[2])) {
			this.drawHoveringText(Arrays.asList(type), mouse_x - screen_coords[0], mouse_y - screen_coords[1]);
		}
		
		String[] side = new String[] { 
				TextHelper.PURPLE + TextHelper.BOLD + "Change connection type" + TextHelper.END, 
				TextHelper.LIGHT_BLUE + "Current State: " + tile.getSide(EnumFacing.UP).getTextColour() + 
		...
		String[] up = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.UP).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.UP).getDisplayName() + "]"
		};
		...
		String[] down = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.DOWN).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.DOWN).getDisplayName() + "]"
		};
		...
		String[] north = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.NORTH).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.NORTH).getDisplayName() + "]"
		};
		...
		String[] south = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.SOUTH).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.SOUTH).getDisplayName() + "]"
		};
		...
		String[] east = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.EAST).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.EAST).getDisplayName() + "]"
		};
		...
		String[] west = new String[] {
				...
				TextHelper.LIGHT_BLUE + "Current Type: " + ((TilePocket) tile_pocket).getSide(EnumFacing.WEST).getTextColour() + 
				"[" + ((TilePocket) tile_pocket).getSide(EnumFacing.WEST).getDisplayName() + "]"
		};
	...
	public void drawStrings(int[] screen_coords) {
		FONT.drawString(fontRenderer, screen_coords, 88, 4, "gui.main.connector.name", true, false, ModGuiUtil.FONT_LIST);

		FONT.drawInventoryString(fontRenderer, screen_coords, 88, 157, false, ModGuiUtil.FONT_LIST);
		
		FONT.drawString(fontRenderer, screen_coords, 93, 36, "gui.header.allowed_players.name", true, false, ModGuiUtil.FONT_LIST);
		FONT.drawString(fontRenderer, screen_coords, 8, 4, "gui.header.config.name", true, false, ModGuiUtil.FONT_LIST);
		FONT.drawString(fontRenderer, screen_coords, 8, 89, "gui.header.surrounding.name", true, false, ModGuiUtil.FONT_LIST);
		FONT.drawString(fontRenderer, screen_coords, 8, 174, "gui.header.internal.name", true, false, ModGuiUtil.FONT_LIST);
	...
	protected void mouseClicked(int mouse_x, int mouse_y, int mouse_button) throws IOException {
		int[] screen_coords = new int[] { (this.width - this.xSize) / 2, (this.height - this.ySize) / 2 };
		int[] weighted_coords = new int[] { (screen_coords[0] + this.TEXT_FIELD.x), (screen_coords[1] + this.TEXT_FIELD.y) };
		
		if (mouse_button == 0) {
			if (IS_HOVERING.pure(mouse_x, mouse_y, weighted_coords[0], weighted_coords[0] + this.TEXT_FIELD.width, weighted_coords[1], weighted_coords[1] + this.TEXT_FIELD.height)) {				
