...
public class CommandRecoverPocketAdmin extends CommandBase {

	@Override
	public String getName() {
		return "recoverpocketadmin";
	}

	@Override
	public String getUsage(ICommandSender sender) {
		return "command.recoverpocket_admin.usage";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1) {
            throw new WrongUsageException("command.recoverpocket_admin.usage", new Object[0]);
        }
		...
		if(args[0] != null || args[0].length() > 0) {
			if(StringUtils.isNumeric(args[0])) {		
				...
				if (map_number <= length) {
					...
					ModUtil.sendPlayerMessage(player, TextHelper.TEAL + "Pocket for [" + pocket.getCreator() + "] recovered.");
				} else {
					throw new WrongUsageException("command.recoverpocket_admin.too.usage", new Object[0]);
				}
