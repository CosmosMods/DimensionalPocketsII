...
public class CommandDimHelp extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if (args.length < 1) {
			...
			TextComponentString command_three = new TextComponentString(TextHelper.LIGHT_GRAY + "/setspawn");
			TextComponentString command_three_desc = new TextComponentString(TextHelper.TEAL + " -- Sets the current position you are standing to the entry point when you enter this pocket.");
			...
			player.sendMessage(command_three);
			player.sendMessage(command_three_desc);
		}
	}
}