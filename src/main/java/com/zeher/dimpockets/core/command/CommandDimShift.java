		...
		if(!args[0].equals(null) || !(args[0].length() == 0)) {
			...
			if(!player.world.equals(null)) {
				if(StringUtils.isNumeric(args[0])) {
					...
					if((ModConfigManager.CAN_TELEPORT_TO_DIM) && dim_id == 98 && !player.isCreative()) {
						TextComponentString string = new TextComponentString(TextHelper.RED + "You cannot teleport to that dimension.");
					...
					Shifter teleporter = ShifterUtil.createTeleporter(dim_id, dim_pos, 0, 0);
					
					if(player.dimension == dim_id) {
						TextComponentString comp = new TextComponentString(TextHelper.RED + "You are already in that dimension!");
						player.sendMessage(comp);
						
						//player.setPositionAndUpdate(spawn_point.getX(), spawn_point.getY(), spawn_point.getZ());
					} else {
						TextComponentString comp = new TextComponentString(TextHelper.TEAL + "Shifted: " + TextHelper.GREEN + player.getName() + TextHelper.TEAL + " to dimension: " + dim_id + ".");
						player.sendMessage(comp);
						
						ShifterUtil.shiftPlayerToDimension(player, dim_id, teleporter);
					...
					DimensionType dimension = DimensionType.getById(-dim_id);
					String dim_name = dimension.getName();
					Shifter teleporter = ShifterUtil.createTeleporter(-dim_id, spawn_point, 0, 0);
