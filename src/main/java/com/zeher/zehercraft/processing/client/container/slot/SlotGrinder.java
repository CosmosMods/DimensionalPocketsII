...
public class SlotGrinder extends Slot {
	...
	public SlotGrinder(EntityPlayer entity_player, IInventory inventory, int index, int x_pos, int y_pos) {
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.player.world.isRemote) {
			int i = this.amount;
			float f = GrinderRecipes.getInstance().getGrindingExperience(stack);
			if (f == 0.0F) {
				i = 0;
			} else if (f < 1.0F) {
