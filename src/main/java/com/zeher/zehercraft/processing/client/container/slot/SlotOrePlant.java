...
public class SlotOrePlant extends Slot {
	...
	public SlotOrePlant(EntityPlayer entity_player, IInventory inventory, int index, int x_pos, int y_pos) {
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.player.world.isRemote) {
			int i = this.amount;
			float f = OrePlantCleaningRecipes.getInstance().getCleaningExperience(stack);
			if (f == 0.0F) {
				i = 0;
			} else if (f < 1.0F) {
			...
			while (i > 0) {
				...
				this.player.world.spawnEntity(new EntityXPOrb(this.player.world, this.player.posX,
						this.player.posY + 0.5D, this.player.posZ + 0.5D, j));
			}
		}
		this.amount = 0;
	}
