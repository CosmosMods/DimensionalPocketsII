...
public class ItemBlockCharger extends AZRFItemBlock {

	public ItemBlockCharger(Block block, String description, String shift_desc_one, String shift_desc_two) {
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hit_x, float hit_y, float hit_z, IBlockState newState) {
		...
		if (state.getBlock().equals(this.block)) {
			if (stack.hasTagCompound()) {
				...
				TileEntityCharger tile_charger = (TileEntityCharger) world.getTileEntity(pos);
				...
				tile_charger.storage.setEnergyStored(energy);
				
				for (int i = 0; i < list_.size(); i++) {
					tile_charger.setInventorySlotContents(i, (ItemStack) list_.get(i));
				}
			}	
		}
		return true;
