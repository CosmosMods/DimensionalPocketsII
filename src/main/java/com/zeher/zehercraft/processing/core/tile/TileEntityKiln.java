...
public class TileEntityKiln extends TileEntity implements ITickable, ISidedInventory, IEnergyReceiver, IEnergyProvider, IClientUpdatedTile {
	...
	public String getName() {
		return this.hasCustomName() ? this.custom_name : ZCReference.RESOURCE.PROCESSING.KILN_NAME;
	}
	...
	public void update() {
		...
		if (storage.getEnergyStored() < storage.getMaxEnergyStored()) {
			...
			//storage.setEnergyStored(30000);
		}
		
		if (this.canProcess() && this.hasEnergy()) {
			
			this.extractEnergy(EnumFacing.DOWN, 20, false);
	...
	public boolean canProcess() {
		if (this.INVENTORY_STACKS.get(0).isEmpty()) {
			...
			ItemStack itemstack = FurnaceRecipes.instance().getSmeltingResult((ItemStack) this.INVENTORY_STACKS.get(0));
