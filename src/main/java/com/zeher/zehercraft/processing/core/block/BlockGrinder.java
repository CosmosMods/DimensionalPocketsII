...
public class BlockGrinder extends BlockContainerRemovableNBT {
	
	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	
	public BlockGrinder(String name, Material material, String tool, int harvest, int hardness, int resistance, CreativeTabs tab) {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hit_x, float hit_y, float hit_z) {
		...
		if (!world.isRemote && !player.isSneaking() && !ModUtil.isHoldingHammer(player)) {
			GuiHandler.openGui(player, ZCReference.RESOURCE.PROCESSING.GRINDER_INDEX, world, pos);
		}
		return false;
	}
