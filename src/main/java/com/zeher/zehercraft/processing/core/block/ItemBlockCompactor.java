...
public class ItemBlockCompactor extends AZRFItemBlock {

	public ItemBlockCompactor(Block block, String description, String shift_desc_one, String shift_desc_two) {
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hit_x, float hit_y, float hit_z, IBlockState newState) {
		...
		if (state.getBlock().equals(this.block)) {
			if (stack.hasTagCompound()) {
				...
				TileEntityCompactor tile = (TileEntityCompactor) world.getTileEntity(pos);
				...
				for (int i = 0; i < list_.size(); i++) {
					tile.setInventorySlotContents(i, (ItemStack) list_.get(i));
				}
			}	
		}
		return true;
