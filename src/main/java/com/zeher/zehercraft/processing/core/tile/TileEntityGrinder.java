...
public class TileEntityGrinder extends TileEntity implements ITickable, ISidedInventory, IEnergyReceiver, IEnergyProvider, IClientUpdatedTile {
	...
	public String getName() {
		return ZCReference.RESOURCE.PROCESSING.GRINDER_NAME;
	}
	...
	public void update() {
		...
		if (this.canProcess() && this.hasEnergy()) {
			...
			if (rand.nextDouble() < 0.1D) {
				//this.world.playSound(pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D, SoundHandler.MACHINE.GRINDER, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
			}
	...
	public boolean canProcess() {
		if (this.INVENTORY_STACKS.get(0).isEmpty()) {
			...
			ItemStack recipeout = GrinderRecipes.getInstance().getGrindingResult(this.INVENTORY_STACKS.get(0));
			ItemStack secondaryout = GrinderRecipes.getInstance().getSecondaryOutput(this.INVENTORY_STACKS.get(0));
	...
	public void processItem() {
		if (this.canProcess()) {
			...
			ItemStack recipeout = GrinderRecipes.getInstance().getGrindingResult(input);
			ItemStack secondaryout = GrinderRecipes.getInstance().getSecondaryOutput(input);
	...
	public boolean canConnectEnergy(EnumFacing from) {
		...
		if (state.getBlock() instanceof BlockGrinder) {
			EnumFacing facing = state.getValue(BlockGrinder.FACING);
