...
public class ItemBlockSeparator extends AZRFItemBlock {

	public ItemBlockSeparator(Block block, String description, String shift_desc_one, String shift_desc_two) {
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hit_x, float hit_y, float hit_z, IBlockState newState) {
		...
		if (state.getBlock().equals(this.block)) {
			if (stack.hasTagCompound()) {
				...
				TileEntitySeparator tile_kiln = (TileEntitySeparator) world.getTileEntity(pos);
