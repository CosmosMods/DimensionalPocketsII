...
public class BlockEnergyChannelSurge extends BlockContainerRemovable {
	...
	public BlockEnergyChannelSurge(String name, Material material, String tool, int harvest, int hardness, int resistance, CreativeTabs tab) {
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileEntityEnergyChannelSurge();
	}
	...
	public void onBlockClicked(World world, BlockPos pos, EntityPlayer player) {
		...
		if (tile instanceof TileEntityEnergyChannelSurge) {
			System.out.println(((TileEntityEnergyChannelSurge) tile).getSideArray());
			
			System.out.println(((TileEntityEnergyChannelSurge) tile).storage.getMaxEnergyStored() + " | " + ((TileEntityEnergyChannelSurge) tile).storage.getEnergyStored() + " | " + ((TileEntityEnergyChannelSurge) tile).getLastFacing());
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hit_x, float hit_y, float hit_z) {
		...
		if (ModUtil.isHoldingHammer(player) && !player.isSneaking()) {
			...
			if (tile instanceof TileEntityEnergyChannelSurge && tile != null) {
				((TileEntityEnergyChannelSurge) tile).cycleSide(facing);
			}
		}
