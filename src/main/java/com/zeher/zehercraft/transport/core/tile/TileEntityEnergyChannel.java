...
public class TileEntityEnergyChannel extends TileEntity implements IChannel.Energy, ITickable, IEnergyReceiver, IEnergyProvider, ChannelType.IChannelEnergy {

	private EnumChannelSideState[] SIDE_STATE_ARRAY = EnumChannelSideState.getStandardArray();
	
	public int capacity = ZCReference.RESOURCE.TRANSPORT.ENERGY_CAPACITY;
	public int max_transfer = ZCReference.RESOURCE.TRANSPORT.ENERGY_MAX_TRANSFER;
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		this.setLastFacing(EnumFacing.getFront(compound.getInteger("last_facing")));
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		if (this.last_facing != null) {
			compound.setInteger("last_facing", this.getLastFacing().getIndex());
		}
	...
	public EnumChannelSideState getStateForConnection(EnumFacing facing) {
		return TransportUtil.getStateForConnection(facing, getPos(), getWorld(), this);
	}
}