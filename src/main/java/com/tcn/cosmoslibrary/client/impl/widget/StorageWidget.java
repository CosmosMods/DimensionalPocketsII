...
public class StorageWidget extends Widget {
	...
	public StorageWidget(int x, int y, int identifier, boolean enabled) {
		this(x, y, 20, identifier, enabled);
	}

	public StorageWidget(int x, int y, int size, int identifier, boolean enabled) {
	...
	public void drawButton(MatrixStack stack, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			minecraft.getTextureManager().bindTexture(CosmosReference.RESOURCE.BASE.GUI_STORAGE_BUTTON_LOC);
