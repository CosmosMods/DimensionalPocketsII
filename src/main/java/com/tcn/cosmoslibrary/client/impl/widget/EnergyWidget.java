...
public class EnergyWidget extends Widget {
	...
	public EnergyWidget(int x, int y, int identifier, boolean enabled) {
		this(x, y, 20, identifier, enabled);
	}

	public EnergyWidget(int x, int y, int size, int identifier, boolean enabled) {
	...
	public void drawButton(MatrixStack stack, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			minecraft.getTextureManager().bindTexture(CosmosReference.RESOURCE.BASE.GUI_ENERGY_BUTTON_LOC);
