...
public class FluidWidget extends Widget {
	...
	public FluidWidget(int x, int y, int identifier, boolean enabled) {
		this(x, y, 20, identifier, enabled);
	}

	public FluidWidget(int x, int y, int size, int identifier, boolean enabled) {
	...
	public void drawButton(MatrixStack stack, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			minecraft.getTextureManager().bindTexture(CosmosReference.RESOURCE.BASE.GUI_FLUID_BUTTON_LOC);
