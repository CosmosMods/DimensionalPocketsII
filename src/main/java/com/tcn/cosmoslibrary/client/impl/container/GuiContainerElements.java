package com.tcn.cosmoslibrary.client.impl.container;
...
public class GuiContainerElements extends ContainerScreen<Container> {
	...
	@Override
	protected void drawGuiContainerForegroundLayer(MatrixStack stack, int mouse_x, int mouse_y) {
		int[] screen_coords = new int[] { ((this.width - this.xSize) / 2), (this.height - this.ySize) / 2 };

		for (int i = 0; i < this.elementlist_array.size(); ++i) {
            this.elementlist_array.get(i).drawElement(stack, this.font, screen_coords, mouse_x, mouse_y, i);
        }
	}
	...
	*/
	...
	@Override
	protected void drawGuiContainerBackgroundLayer(MatrixStack stack, float partial_ticks, int x, int y) { }
