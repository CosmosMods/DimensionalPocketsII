...
import com.tcn.cosmoslibrary.energy.item.CosmosEnergyAxe;
...
public class DimensionalAxe extends CosmosEnergyAxe {
	
	public boolean enhanced;

	public DimensionalAxe(Tier itemTier, int attackDamageIn, float attackSpeedIn, boolean enhanced, Properties builderIn, CosmosEnergyItem.Properties energyProperties) {
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		if (!ComponentHelper.isShiftKeyDown(Minecraft.getInstance())) {
			if (this.enhanced) {
				tooltip.add(ComponentHelper.getTooltipInfo("dimensionalpocketsii.item.info.axe_enhanced"));
			} else {
				tooltip.add(ComponentHelper.getTooltipInfo("dimensionalpocketsii.item.info.axe"));
			}
