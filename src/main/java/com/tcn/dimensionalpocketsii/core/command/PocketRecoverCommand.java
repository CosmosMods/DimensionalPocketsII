			...
			if (pocketIn.exists()) {
				...
				if (playerInformation != null) {
					if (playerDisplayName.equals(playerInformation.getPlayerName()) && playerUUID.equals(playerInformation.getPlayerUUID())) {
						...
						commandSourceIn.sendFailure(CosmosCompHelper.locComp("dimensionalpocketsii.command.recoverpocket.error.not_owner"));
					}
				} else {
					ItemStack pocketStack = pocketIn.generateItemStackWithNBT();
					
					if (serverPlayer.inventory.add(pocketStack)) {
						commandSourceIn.sendSuccess(CosmosCompHelper.locComp(CosmosColour.GREEN, false, "dimensionalpocketsii.command.recoverpocket.success.single_no_owner", ": [ " + chunkPos.getX() + ", " + chunkPos.getZ() + " ]"), true);
						System.out.println("Player: [ " + playerDisplayName + ", " + playerUUID + " ] Just activated the Recover Pocket Command. For Pocket: " + chunkPos);
					} else {
						commandSourceIn.sendFailure(CosmosCompHelper.locComp("dimensionalpocketsii.command.recoverpocket.error.no_space"));
					}
