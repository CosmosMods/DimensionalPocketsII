...
public class ItemBlockEnergyDisplay extends BlockItem {

	public ItemBlockEnergyDisplay(Block block, Item.Properties prop) {
		super(block, prop);
	}
	...
	public void appendHoverText(ItemStack stackIn, @Nullable World worldIn, List<ITextComponent> toolTipIn, ITooltipFlag flagIn) {
		...
		if (!CosmosCompHelper.isShiftKeyDown(Minecraft.getInstance())) {
			toolTipIn.add(CosmosCompHelper.getTooltipInfo("dimensionalpocketsii.info.block.module.energy_display"));
		
			if (CosmosCompHelper.displayShiftForDetail) {
				toolTipIn.add(CosmosCompHelper.shiftForMoreDetails());
			}
		} else {
			toolTipIn.add(CosmosCompHelper.getTooltipOne("dimensionalpocketsii.info.block.module.energy_display_shift_one"));
