...
public class ItemBlockConnector extends BlockItem {
	...
	public ItemBlockConnector(Block block, Item.Properties prop) {
		super(block, prop);
	}
	...
	public void appendHoverText(ItemStack stackIn, @Nullable World worldIn, List<ITextComponent> toolTipIn, ITooltipFlag flagIn) {
		...
		if (!CosmosCompHelper.isShiftKeyDown(Minecraft.getInstance())) {
			toolTipIn.add(CosmosCompHelper.getTooltipInfo("dimensionalpocketsii.info.block.module.connector"));
			
			if (CosmosCompHelper.displayShiftForDetail) {
				toolTipIn.add(CosmosCompHelper.shiftForMoreDetails());
			}
		} else {
			toolTipIn.add(CosmosCompHelper.getTooltipOne("dimensionalpocketsii.info.block.module.connector_shift_one"));
			toolTipIn.add(CosmosCompHelper.getTooltipTwo("dimensionalpocketsii.info.block.module.connector_shift_two"));
