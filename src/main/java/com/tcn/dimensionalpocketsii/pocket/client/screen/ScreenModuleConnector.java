...
@OnlyIn(Dist.CLIENT)
public class ScreenModuleConnector extends CosmosScreenUIModeList<ContainerModuleConnector> {
	...
	private CosmosButtonWithType buttonHostileSpawn; private int[] HSBI = new int[] { 208, 130 };
	
	private CosmosButtonWithType buttonPlaceHolder0; private int[] PB0I = new int[] { 231, 130 };
	
	public ScreenModuleConnector(ContainerModuleConnector containerIn, Inventory playerInventoryIn, Component titleIn) {
		super(containerIn, playerInventoryIn, titleIn);

		this.setImageDims(360, 256);
		
		this.setLight(RESOURCE.CONNECTOR[0]);
		this.setDark(RESOURCE.CONNECTOR[1]);
		
		this.setDualScreen();
		this.setDualScreenIndex(256, 0, 104, 256);
		
		this.setDualLight(RESOURCE.CONNECTOR_SIDE[0]);
		this.setDualDark(RESOURCE.CONNECTOR_SIDE[1]);
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks){
		...
		if (entity instanceof BlockEntityModuleConnector) {
			...
			if (blockEntity.getPocket() != null) {
				...
				FONT.drawString(poseStack, font, this.getScreenCoords(), 88, 4, true, ComponentHelper.locComp(textColour, false, "dimensionalpocketsii.gui.connector.header"));
	...
	protected void renderBg(PoseStack poseStack, float partialTicks, int mouseX, int mouseY) {
		...
		if (entity instanceof BlockEntityModuleConnector) {
			...
			if (blockEntity.getPocket() != null) {
				...
				CosmosUISystem.renderStaticElementWithUIMode(this, poseStack, this.getScreenCoords(), 0, 0, 0, 0, 256, 256, new float[] { rgb[0], rgb[1], rgb[2], 1.0F }, blockEntity, GUI.RESOURCE.CONNECTOR_BASE_NORMAL);
				CosmosUISystem.renderStaticElementWithUIMode(this, poseStack, this.getScreenCoords(), 256, 0, 0, 0, 104, 256, new float[] { rgb[0], rgb[1], rgb[2], 1.0F }, blockEntity, GUI.RESOURCE.CONNECTOR_BASE_SIDE);
				
				CosmosUISystem.renderStaticElementWithUIMode(this, poseStack, this.getScreenCoords(), 0, 0, 0, 0, 256, 256, blockEntity, GUI.RESOURCE.CONNECTOR_OVERLAY_NORMAL);
				CosmosUISystem.renderStaticElementWithUIMode(this, poseStack, this.getScreenCoords(), 256, 0, 0, 0, 104, 256, blockEntity, GUI.RESOURCE.CONNECTOR_OVERLAY_SIDE);
		...
		if (entity instanceof BlockEntityModuleConnector) {
			...
			if (blockEntity.getPocket() != null) {
				...
				for (Direction c : Direction.values()) {
					sides[c.get3DDataValue()] = (pocket.getSideArray()[c.get3DDataValue()].getIndex());
				}
				...
				this.buttonSideState = this.addRenderableWidget(new DimensionalButton(this.getScreenCoords()[0] + SIBI[0], this.getScreenCoords()[1] + SIBI[1], 20, true, true, (blockEntity.getSide(Direction.UP).getIndex() + 8), ComponentHelper.locComp(""), (button) -> { this.pushButton(this.buttonSideState); }));
				this.buttonType = this.addRenderableWidget(new DimensionalButton(this.getScreenCoords()[0] + TYBI[0], this.getScreenCoords()[1] + TYBI[1], 20, true, true, (blockEntity.getConnectionType().getIndex() + 4), ComponentHelper.locComp(""), (button) -> { this.pushButton(this.buttonType); }));
			...
			if (blockEntity.getPocket() != null) {
				...
				if (pocket != null) {
					if (this.buttonLock.isMouseOver(mouseX, mouseY)) {
						...
						Component[] comp = new Component[] { ComponentHelper.locComp(ComponentColour.WHITE, false, "dimensionalpocketsii.gui.connector.connection_info"), 
								ComponentHelper.locComp(ComponentColour.GRAY, false, "dimensionalpocketsii.gui.connector.connection_value").append(blockEntity.getConnectionType().getColouredComp()) };
						
						this.renderComponentTooltip(poseStack, Arrays.asList(comp), mouseX, mouseY);
					} else if (this.buttonSideState.isMouseOver(mouseX, mouseY)) {
						Component[] comp = new Component[] { ComponentHelper.locComp(ComponentColour.WHITE, false, "dimensionalpocketsii.gui.connector.side_state_info"), 
								ComponentHelper.locComp(ComponentColour.GRAY, false, "dimensionalpocketsii.gui.connector.side_state_value").append(blockEntity.getSide(Direction.UP).getColouredComp()) };
		...
		if (entity instanceof BlockEntityModuleConnector) {
			...
			if (blockEntity.getPocket() != null) {
				...
				if (button.equals(this.buttonType)) {
					PocketNetworkManager.sendToServer(new PacketConnectionType(this.menu.getBlockPos()));
					...
					PocketNetworkManager.sendToServer(new PacketSideState(this.menu.getBlockPos()));
	...
	protected void addUIHelpElements() {
		...
		this.addRenderableUIHelpElement(this.getScreenCoords(), 34, 175, 20, 68, ComponentHelper.locComp(ComponentColour.ORANGE, false, "dimensionalpocketsii.gui.help.fluid_display"), 
		...
		this.addRenderableUIHelpElement(this.getScreenCoords(), 55, 175, 22, 22, ComponentHelper.locComp(ComponentColour.LIGHT_BLUE, false, "dimensionalpocketsii.gui.help.slot.bucket_input"), 
		...
		this.addRenderableUIHelpElement(this.getScreenCoords(), 55, 198, 22, 22, ComponentHelper.locComp(ComponentColour.GREEN, false, "dimensionalpocketsii.gui.help.slot.bucket_output"), 
