	...
	protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
		...
		if (entity instanceof BlockEntityModuleFurnace) {
			...
			if (this.menu.isLit()) {
				...
				graphics.blit(blockEntity.getUIMode().equals(EnumUIMode.DARK) ? GUI.RESOURCE.FURNACE_OVERLAY[1] : GUI.RESOURCE.FURNACE_OVERLAY[1], this.getScreenCoords()[0] + 53, this.getScreenCoords()[1] + 36 + 13 - k, 184, 12 - k, 14, k + 1);
			}

			int l = this.menu.getBurnProgress();
			graphics.blit(blockEntity.getUIMode().equals(EnumUIMode.DARK) ? GUI.RESOURCE.FURNACE_OVERLAY[1] : GUI.RESOURCE.FURNACE_OVERLAY[1], this.getScreenCoords()[0] + 75, this.getScreenCoords()[1] + 34, 184, 14, l + 1, 16);
		}
	}
