...
public class ContainerPocketEnhanced extends CosmosContainerMenuBlockEntity {
	...
	public static ContainerPocketEnhanced createContainerServerSide(int windowID, Inventory playerInventory, Container pocketIn, Container contentsIn, BlockPos pos) {
		return new ContainerPocketEnhanced(windowID, playerInventory, pocketIn, contentsIn, pos);
	}
	...
	public static ContainerPocketEnhanced createContainerClientSide(int windowID, Inventory playerInventory, FriendlyByteBuf extraData) {
		...
		return new ContainerPocketEnhanced(windowID, playerInventory, new SimpleContainer(DimReference.CONSTANT.POCKET_HELD_ITEMS_SIZE_WITH), new SimpleContainer(DimReference.CONSTANT.POCKET_HELD_ITEMS_SIZE_WITH + 2), pos);
	}
	...
	protected ContainerPocketEnhanced(int id, Inventory playerInventoryIn, Container pocketIn, Container contentsIn, BlockPos posIn) {
		super(ObjectManager.container_pocket_enhanced, id, playerInventoryIn, null, posIn);
