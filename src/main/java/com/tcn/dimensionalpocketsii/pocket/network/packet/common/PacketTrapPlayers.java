...
public class PacketTrapPlayers  {
	...
	public PacketTrapPlayers(PacketBuffer buf) {
	...
	public PacketTrapPlayers(BlockPos pos, RegistryKey<World> dimension, boolean lock) {
		this.pos = pos;
		this.trap = lock;
		this.dimension = dimension;
	}
	
	public static void encode(PacketTrapPlayers packet, PacketBuffer buf) {
	...
	public static void handle(final PacketTrapPlayers packet, Supplier<NetworkEvent.Context> context) {
		...
		ctx.enqueueWork(() -> {
			...
			if (tile instanceof TileEntityConnector) {
				...
				if (pocket.exists()) {
					pocket.setTrapState(packet.trap);
