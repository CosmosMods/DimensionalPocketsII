...
public class PacketLock  {
	...
	public PacketLock(BlockPos pos, ResourceKey<Level> dimension, boolean lock) {
		this.pos = pos;
		this.lock = lock;
		...
		ctx.enqueueWork(() -> {
			...
			if (tile instanceof BlockEntityModuleConnector) {
				...
				if (pocket.exists()) {
					...
					DimensionalPockets.CONSOLE.debug("[Packet Delivery Success] {connector} <lock> Lock setting set to: { " + value +  " } for Pocket: { " + pocket.getChunkPos() + "} ");
				}
				...
				if (pocket.exists()) {
					...
					DimensionalPockets.CONSOLE.debug("[Packet Delivery Success] {pocket} <lock> Lock setting set to: { " + value +  " } for Pocket: { " + pocket.getChunkPos() + "} ");
				}
			} else {
				DimensionalPockets.CONSOLE.debugWarn("[Packet Delivery Failure] <lock> Block Entity not equal to expected.");
			}
