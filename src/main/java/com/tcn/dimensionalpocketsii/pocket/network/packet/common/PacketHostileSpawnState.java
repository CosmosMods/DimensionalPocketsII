...
public class PacketHostileSpawnState  {
	...
	public PacketHostileSpawnState(PacketBuffer buf) {
	...
	public PacketHostileSpawnState(BlockPos pos, RegistryKey<World> dimension, boolean lock) {
		this.pos = pos;
		this.lock = lock;
		this.dimension = dimension;
	}
	
	public static void encode(PacketHostileSpawnState packet, PacketBuffer buf) {
	...
	public static void handle(final PacketHostileSpawnState packet, Supplier<NetworkEvent.Context> context) {
		...
		ctx.enqueueWork(() -> {
			...
			if (tile instanceof TileEntityConnector) {
				...
				if (pocket.exists()) {
					pocket.setHostileSpawnState(packet.lock);
