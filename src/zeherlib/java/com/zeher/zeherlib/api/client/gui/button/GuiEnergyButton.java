...
public class GuiEnergyButton extends GuiButton {
	...
	public GuiEnergyButton(int buttonId, int x, int y, int identifier, boolean enabled) {
		this(buttonId, x, y, 20, identifier, enabled);
	}

	public GuiEnergyButton(int buttonId, int x, int y, int size, int identifier, boolean enabled) {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			mc.getTextureManager().bindTexture(ZLReference.RESOURCE.BASE.GUI_ENERGY_BUTTON_LOC);
