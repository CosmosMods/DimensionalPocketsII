...
public class GuiItemButton extends Widget {
	...
	public GuiItemButton(int x, int y, int identifier, boolean enabled) {
		this(x, y, 20, identifier, enabled);
	}

	public GuiItemButton(int x, int y, int size, int identifier, boolean enabled) {
		super(x, y, size, size, "");
		this.width = size;
		this.height = size;
		this.size = size;
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			mc.getTextureManager().bindTexture(ZLReference.RESOURCE.BASE.GUI_ITEM_BUTTON_LOC);
